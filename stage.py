...
class Stage:
    ...
    def __can_rotation_block(self):
        ...
        # 回転することが出来るのであればTrueを返却し、そうでなければFalseを返却します。
        # 次の角度
    ...
    def __fix_block(self):
        ...
        for i in range(block.Block.SIZE):
            for j in range(block.Block.SIZE):
                if self.block.get_cell_data(self.type, self.rot, j, i) == Stage.BLOCK:
    ...
    def is_collision_bottom(self, x = -1, y = -1):
        # 下方向の衝突判定を行うメソッドです。
        # 衝突していればTrue, そうでなければFalseが返却されます。
        # x: 対象のブロックのX軸座標
        # y: 対象のブロックのY軸座標
        ...
        for i in range(block.Block.SIZE):
            for j in range(block.Block.SIZE):
                ...
                if self.block.get_cell_data(b_t, b_r, j, i) == Stage.BLOCK:
                    # 対象のブロックますの位置から１つ下げたマスが
                    # ステージの範囲外だった場合
                    if self.is_out_of_stage(x + j, y + i + 1):
                        return True
                    # 対象のブロックマスの位置から1つ下げたマスが
    ...
    def is_collision_left(self, x = -1, y = -1):
        ...
        for i in range(block.Block.SIZE):
            for j in range(block.Block.SIZE):
                # 取得したブロックの1マスのデータがBLOCK(1)だった場合
                if self.block.get_cell_data(b_t, b_r, j, i) == Stage.BLOCK:
                    # 対象のブロックますの位置から１つ左のマスが
                    # ステージの範囲外だった場合
                    if self.is_out_of_stage(x + j - 1, y + i):
                        return True
                    # 対象のブロックマスの位置から1つ左のマスが
                    # 固定されたブロックのマス(2)だった場合
    ...
    def is_collision_right(self, x = -1, y = -1):
        ...
        for i in range(block.Block.SIZE):
            for j in range(block.Block.SIZE):
                # 取得したブロックの1マスのデータがBLOCK(1)だった場合
                if self.block.get_cell_data(b_t, b_r, j, i) == Stage.BLOCK:
                    # 対象のブロックますの位置から１つ右のマスが
                    # ステージの範囲外だった場合
                    if self.is_out_of_stage(x + j + 1, y + i):
                        return True
                    # 対象のブロックマスの位置から1つ右のマスが
                    # 固定されたブロックのマス(2)だった場合
                    if self.data[y + i][x + j + 1] == Stage.FIX:
                        return True
        # どの条件にも当てはまらない場合は常にどこにも衝突していない
    ...
    def __remove_lines(self):
        ...
        for i in reversed(range(Stage.HEIGHT)): # 19から0に向かう
            # もし、対象の列が揃っていなかった場合
            if not self.remove_line[i]:
                for j in range(Stage.WIDTH):
                    # 置き換え先の列に揃っていない列を代入する
                    self.data[idx][j] = self.data[i][j] # 例data[18][j] = data[19][j]
                # 置き換え先の列を参照するポインタを1列上げる。
                idx -= 1
    ...
    def is_end(self):
        ...
        # ゲームオーバーであればTrueを返却し、そうでなければFalseを返却します。
        # ブロックの 情報を取得
        t = self.type
        r = self.rot
        x = self.block.x
